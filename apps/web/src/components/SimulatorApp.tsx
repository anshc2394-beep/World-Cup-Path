...
export function SimulatorApp({ section = "all", initialGroup = "A" }: { section?: Section; initialGroup?: string }) {
  ...
  const [group, setGroup] = useState(initialGroup.toUpperCase());
  ...
  return <div className="shell py-10">
    <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><p className="eyebrow">Official pre-tournament snapshot</p><h1 className="font-display mt-2 text-4xl font-bold md:text-6xl">Build your path to the trophy</h1><p className="muted mt-2">{state.progress.completed_group_matches} of {state.progress.total_group_matches} group matches complete</p></div><div className="flex flex-wrap gap-2"><button className="button button-secondary" onClick={() => { reset(); setState(null); }}><RotateCcw size={16}/> Reset</button><button className="button button-secondary" disabled={busy} onClick={simulate}><Sparkles size={16}/> {busy ? "Simulating…" : "Simulate tournament"}</button><button className="button button-primary" disabled={busy} onClick={save}><Save size={16}/> Save & share</button></div></div>
    ...
    {(section === "all" || section === "groups") && <section className="card p-4 md:p-6"><div className="mb-5 flex gap-2 overflow-x-auto pb-1" role="tablist" aria-label="World Cup groups">{"ABCDEFGHIJKL".split("").map((id) => <button role="tab" aria-selected={group === id} key={id} onClick={() => setGroup(id)} className={`min-w-10 cursor-pointer rounded-lg px-3 py-2 font-data text-sm transition-colors ${group === id ? "bg-blue-600 text-white" : "bg-slate-900 text-slate-400 hover:text-white"}`}>{id}</button>)}</div><div className="grid min-w-0 gap-6 xl:grid-cols-[1fr_1.15fr]"><div className="min-w-0"><h2 className="font-display mb-3 text-2xl font-bold">Group {group} matches</h2><div className="space-y-2">{groupFixtures.map((fixture) => <MatchCard key={fixture.id} fixture={fixture} teams={teamMap} score={scores[fixture.id]} onScore={(side, value) => setScore(fixture.id, side, value)}/>)}</div></div><GroupTable group={group} rows={state.standings[group]} teams={teamMap}/></div></section>}
